...
def request(url: str, method: str = 'GET', session: Session = requests.Session()) -> Response:
    for _ in range(60):
        try:
            response = session.request(method=method, url=url, timeout=60.0, headers=HEADERS)
